...
function sessionDir(root: string, sessionId: string) {
  return path.join(root, ".opencode", "irving", sessionId);
}
...
export function createPipelineTools(worktree: string): Record<string, ToolDefinition> {
  return {
    pipeline_init: tool({
      description: "Create a new session with a generated ID. Returns the session ID.",
      args: {},
      async execute() {
    ...
    pipeline_read_state: tool({
      ...
      async execute(args) {
        return JSON.stringify(await readStateFile(worktree, args.session_id), null, 2);
      },
    }),
    ...
    pipeline_set_phase: tool({
      ...
      async execute(args) {
        const state = await readStateFile(worktree, args.session_id);
        state.phase = args.phase;
        await writeStateFile(worktree, args.session_id, state);
        await logEvent(worktree, args.session_id, { type: "pipeline.phase", phase: args.phase });
    ...
    pipeline_set_planning_status: tool({
      ...
      async execute(args) {
        ...
        await writeStateFile(worktree, args.session_id, state);
        await logEvent(worktree, args.session_id, { type: "pipeline.planning_status", status: args.status, round: state.planning.round });
    ...
    pipeline_set_execution_status: tool({
      ...
      async execute(args) {
        ...
        await writeStateFile(worktree, args.session_id, state);
        await logEvent(worktree, args.session_id, { type: "pipeline.execution_status", status: args.status, iteration: state.execution.iteration });
    ...
    pipeline_record_evidence: tool({
      ...
      async execute(args) {
        ...
        await writeStateFile(worktree, args.session_id, state);
        await logEvent(worktree, args.session_id, { type: "pipeline.evidence", ac_id: args.ac_id, evidence_type: args.type, detail: args.detail });
    ...
    pipeline_ignore_finding: tool({
      ...
      async execute(args) {
        ...
        await writeStateFile(worktree, args.session_id, state);
        await logEvent(worktree, args.session_id, { type: "pipeline.ignore_finding", finding_id: args.finding_id, reason: args.reason });
    ...
    pipeline_set_active_work_units: tool({
      ...
      async execute(args) {
        ...
        await writeStateFile(worktree, args.session_id, state);
        await logEvent(worktree, args.session_id, { type: "pipeline.active_wu", ids: args.ids });
    ...
    pipeline_set_blocked_work_units: tool({
      ...
      async execute(args) {
        ...
        await writeStateFile(worktree, args.session_id, state);
        await logEvent(worktree, args.session_id, { type: "pipeline.blocked_wu", ids: args.ids });
    ...
    pipeline_create_plan: tool({
      ...
      async execute(args) {
        await ensureDirs(worktree, args.session_id);
        const file = path.join(sessionDir(worktree, args.session_id), "plan.json");
        await writeFile(file, args.plan, "utf8");
        await logEvent(worktree, args.session_id, { type: "pipeline.plan_created" });
        return `Wrote ${file}`;
      },
    }),
    ...
    pipeline_create_work_unit_file: tool({
      ...
      async execute(args) {
        await ensureDirs(worktree, args.session_id);
        const file = path.join(sessionDir(worktree, args.session_id), "work-units", `${args.id}.md`);
        await writeFile(file, args.content, "utf8");
        await logEvent(worktree, args.session_id, { type: "pipeline.work_unit.created", id: args.id });
        return `Wrote ${file}`;
      },
    }),
    ...
    pipeline_append_human_context: tool({
      ...
      async execute(args) {
        ...
        const file = path.join(sessionDir(worktree, args.session_id), "debate", name);
        await appendFile(file, args.content + "\n\n", "utf8");
        await logEvent(worktree, args.session_id, { type: "pipeline.human_context", round: args.round });
    ...
    pipeline_record_decision: tool({
      ...
      async execute(args) {
        await logEvent(worktree, args.session_id, { type: "pipeline.decision", decision: args.decision, reasoning: args.reasoning, work_unit: args.work_unit ?? null });
    ...
    pipeline_set_next_action: tool({
      ...
      async execute(args) {
        ...
        await writeStateFile(worktree, args.session_id, state);
        await logEvent(worktree, args.session_id, { type: "pipeline.next_action", next_action: args.next_action, reason: args.reason, iteration: state.execution.iteration });
